<x-guest-layout>
    <div class="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6">
        <div class="text-center mb-8">
            ...
            <p class="mt-2 text-slate-500">{{ __('app.auth.healthcare_workspace') }}</p>
        </div>

        <div class="w-full max-w-xl bg-white border border-slate-200 rounded-2xl shadow-xl p-8 sm:p-10">
            <div class="mb-8">
                <h2 class="text-3xl font-extrabold text-slate-950">{{ __('app.auth.create_account') }}</h2>
                <p class="mt-2 text-slate-500">{{ __('app.auth.register_subtitle') }}</p>
            </div>
            ...
            <form method="POST" action="{{ route('register') }}" class="space-y-5">
                ...
                <div>
                    <label for="name" class="block text-sm font-bold text-slate-700 uppercase tracking-widest mb-2">{{ __('app.auth.full_name') }}</label>
                    <div class="relative">
            ...
            <div class="mt-8 pt-6 border-t border-slate-200 text-center text-slate-600">
                ...
                <a href="{{ route('login') }}" class="font-bold text-blue-700 hover:text-blue-800">{{ __('app.auth.login') }}</a>
            </div>
        </div>
        ...
        <div class="mt-8 flex flex-wrap items-center justify-center gap-4 text-xs font-bold uppercase tracking-widest text-slate-600">
            <span class="px-4 py-2 rounded-full bg-white border border-slate-200">{{ __('app.auth.systems_secure') }}</span>
            <span class="px-4 py-2 rounded-full bg-white border border-slate-200">{{ __('app.auth.clinic_ready') }}</span>
        </div>
    </div>
</x-guest-layout>
